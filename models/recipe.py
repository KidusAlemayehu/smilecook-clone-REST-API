...
class Recipe(db.Model):
    ...
    no_of_servings = db.Column(db.Integer)
    ...
    def get_all_published(cls, q, page, per_page, sort, order):
        ...
        return cls.query.filter(or_(cls.name.ilike(keyword), cls.description.ilike(keyword), cls.ingredients.ilike(keyword)), cls.is_publish.is_(True)).order_by(sort_logic).paginate(page=page, per_page=per_page)
    ...
    @classmethod
    def get_all_by_user(cls, user_id, page, per_page, visibility):
        query = cls.query.filter_by(user_id=user_id)
        if visibility == 'public':
            query = cls.query.filter_by(user_id=user_id, is_publish=True)
        elif visibility == 'private':
            query = cls.query.filter_by(
                user_id=user_id, is_publish=False)
